...
anything else — the image bundles them all. From inside `profiler/` (where
`run.sh` lives — the folder you run `./run.sh` from is what gets mounted for results):
...
./run.sh --organization https://github.com/your-org
GITLAB_TOKEN=glpat_xxx ./run.sh --organization my-group --platform gitlab
GITLAB_TOKEN=glpat_xxx ./run.sh --organization https://gitlab.com/your-group
BITBUCKET_USERNAME=you BITBUCKET_TOKEN=xxx ./run.sh --organization your-workspace/PROJ --platform bitbucket
...
**5. Set up this project.** This tool is the `profiler` subpackage of the
bigger `org-analyser` project — install from the repo root, not this folder:
...
(Or `python -m pip install -e .` if you're not using `uv` — see the repo-root `README.md`.)

You're ready. Jump to [Usage](#usage). Every `uv run codebase-profiler ...` / `codebase-profiler ...`
command below works the same regardless of which folder you run it from, once installed.
...
# One repo
uv run codebase-profiler --repo your-workspace/example-repo --platform bitbucket

# Whole Bitbucket project (all repos in project PROJ)
uv run codebase-profiler --organization https://bitbucket.org/your-workspace/workspace/projects/PROJ --platform bitbucket

# Or shorthand
uv run codebase-profiler --organization your-workspace/PROJ --platform bitbucket

# All repos in a workspace
uv run codebase-profiler --organization your-workspace --platform bitbucket
```

For **Bitbucket Data Center / Server**, pass `--host your-bitbucket.company.com` and use
`--organization PROJECT_KEY` (e.g. `PROJ`) with a personal access token that can read
...
uv run codebase-profiler ~/Coding/example-project --print -v
uv run codebase-profiler ~/Coding/example-project --no-github   # purely local

# --- Single remote repo --------------------------------------------------
# owner/name or a full URL; platform is inferred from a URL, else --platform
uv run codebase-profiler --repo your-org/example-repo
...
uv run codebase-profiler --organization your-org --out ~/Downloads/your-org.xlsx
