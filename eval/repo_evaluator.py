...
try:
    ...
    from eval.repo_evaluator_helpers import (
        MAX_ISSUE_WORDS,
        MIN_ISSUE_WORDS,
        count_words,
        get_language_config,
...
try:
    ...
    from eval.quality_checks import run_all_quality_checks

try:
    from .taxonomy_check import TAXONOMY_COLUMNS, run_taxonomy_for_accepted_prs
except Exception:
    from eval.taxonomy_check import TAXONOMY_COLUMNS, run_taxonomy_for_accepted_prs

try:
    from .training_data_quality_check import analyze_training_data_quality
except Exception:
    from eval.training_data_quality_check import analyze_training_data_quality
...
class RepoMetrics:
    ...
    eval_contamination_files: Optional[int] = None
    eval_contamination_detail: List[str] = None
    training_suitability: Optional[str] = None
    training_quality_score: Optional[float] = None
    training_quality_reasons: List[str] = None
...
class PRAnalyzer:
    ...
    def _get_patch_from_git(
        ...
        try:
            from eval.repo_evaluator_helpers import get_full_patch_content
...
class RepoEvaluator:
    ...
    def evaluate(self) -> AnalysisReport:
        ...
        try:
            training_quality = analyze_training_data_quality(
                self.repo_path, has_tests=repo_metrics.test_files > 0
            )
            repo_metrics.license = training_quality["license"]
            repo_metrics.license_risk = training_quality["license_risk"]
            repo_metrics.eval_contamination_files = training_quality["eval_contamination_files"]
            repo_metrics.eval_contamination_detail = training_quality["eval_contamination_detail"]
            repo_metrics.training_suitability = training_quality["training_suitability"]
            repo_metrics.training_quality_score = training_quality["training_quality_score"]
            repo_metrics.training_quality_reasons = training_quality["training_quality_reasons"]
        except Exception as e:
            logger.warning(f"Training-data-quality analysis failed: {e}")
    ...
    def _run_f2p_analysis(
        ...
        from eval.test_runners import F2PP2PAnalyzer, get_runner, preflight_check
...
def to_json(report: AnalysisReport) -> dict:
    ...
    repo_metrics_out = {
        ...
        "license": repo_metrics_json.get("license"),
        "license_risk": repo_metrics_json.get("license_risk"),
        "eval_contamination_files": repo_metrics_json.get("eval_contamination_files"),
        "eval_contamination_detail": repo_metrics_json.get("eval_contamination_detail"),
        "training_suitability": repo_metrics_json.get("training_suitability"),
        "training_quality_score": repo_metrics_json.get("training_quality_score"),
        "training_quality_reasons": repo_metrics_json.get("training_quality_reasons"),
    }
