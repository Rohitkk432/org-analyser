...
**Posture in one paragraph.** The pipeline is safe to run **on codebases you own** (§5.1). Its two structural risks — untrusted repository code executing with your credentials, and repository content reaching third-party LLMs — are both closed by controls that hold at a choke point rather than by convention, and both are covered by tests (§4, §8). Developer personal data is captured but pseudonymised before it reaches any deliverable, and run bundles now expire. Two items remain open and **neither is a code change**: confirm your Gemini API key's tier, and — if the developers being analysed are staff — cover this tool in your internal engineering-tooling notice (§7).
...
### 5.2 What ownership does not settle

One distinction does the work. The operator consents as the **owner of the repository**. But the personal data in §2.2 — committer names, review comments, per-author statistics — belongs to the **individual developers who wrote it**. They are the data subjects, and they are not the person running the tool. Repo ownership does not transfer their rights.

So the surviving obligations are about the developers, not the code:
...
| Processors / transfers (Art. 28, Ch. V) | **Confirm once.** Standard API DPAs almost certainly cover this — but confirm the **Gemini tier** (§3). Free-tier submission is a disclosure the operator *cannot* authorise on their developers' behalf. |
| Transparency (Arts. 13-14) | **Open if staff.** If the analysed developers are employees or contractors, they should know their commit history and review comments are scored and sent to third-party LLMs. |
...
Neither of these is a code change. Both are yours.

1. **Confirm the Gemini API key's tier.** If it is free-tier, Google is using your source code for product improvement (§3). This is the highest-consequence unknown in the system.
2. **Staff notice.** If the analysed developers are employees or contractors, cover this tool in the internal engineering-tooling notice (§5.2). Low effort, and the thing most likely to cause friction if skipped.
3. **Historical run bundles predate the email fix.** Bundles already in `outputs/org-pipeline-runs/` still contain real committer addresses — §4.6 applies to new runs only. Purge them or re-run.
...
`test_env_scrub.py` seeds five fake credentials and spawns a child process that dumps its own environment, asserting none of the values survive — an end-to-end test of the actual attack, not a unit test of the allowlist. `test_llm_redaction.py` drives a fake OpenAI client and asserts that what reaches the provider contains no key material, *and* that the surrounding prose and diff structure do survive, so classification quality is unchanged.

Run both after any change to the LLM call sites, the redactor, or the test runners.

**Not yet done:** a dependency CVE scan, and a dynamic test against a deliberately hostile repository (a clone whose `conftest.py` phones home). Both are worth doing before this is pointed at anything less trusted than your own code.
